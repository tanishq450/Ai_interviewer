...
function stopRecording() {
  ...
  $('mic-hint').textContent = '⏳ Processing…';

  cancelAnimationFrame(appState.waveAnimId);
  clearWaveform();
...
async function handleRecordingStop() {
  const mimeType = appState.mediaRecorder?.mimeType || 'audio/webm';
  ...
  try {
    showLoader('Submitting voice answer…');
    const res = await fetch(`${API}/interview/answer-voice`, { method: 'POST', body: fd });
    const data = await res.json();
    if (!res.ok) throw new Error(data.detail || 'Voice submit failed');

    handleAnswerResponse(data, data.transcript || '(voice answer)');
  } catch (err) {
    hideLoader();
